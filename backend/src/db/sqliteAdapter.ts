...
export class SQLiteAdapter implements IDatabase {
  ...
  getBandsByGenre(genre: string): Band[] {
    ...
    return rows.map(row => this.rowToBand(row));
  }
  ...
  updateBandDescription(id: string, description: string): void {
    const stmt = this.db.prepare('UPDATE bands SET description = ? WHERE id = ?');

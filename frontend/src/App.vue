...
const seedBand = ref<string | undefined>(undefined);

const onGenreSelected = async (genre: string, seedBandName?: string) => {
  try {
    // 获取当前 LLM 配置
    ...
    // 保存种子乐队
    seedBand.value = seedBandName;
    // 创建 session，传递 LLM 配置和种子乐队
    sessionId.value = await apiService.createSession(genre, llmConfig, seedBandName);
